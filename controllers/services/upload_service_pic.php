    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($auth['success'] == 1){
            ...
            if($error != 1){
                ...
                $service_dir = __DIR__ . "/../../storage/".$acc_id.'/service/'.$service_id.'/';  
                
                $valid_extension = array('png', 'jpg', 'jpeg');
                $file_extension = strtolower(pathinfo($file_name, PATHINFO_EXTENSION)); // get uploaded image file extension

                if(in_array($file_extension, $valid_extension)){
                    // check if the file extension is valid
                    try{
                        if(move_uploaded_file($path, $service_dir.$file_name)){ // move uploaded file to service directory
                            http_response_code(200);
                            $returnData = msg(0, 200, 'Image Uploaded Successfully');
                        }
                        else{
                            http_response_code(400);
                            $returnData = msg(0, 400, 'Uploading Failed', array( "path" => $path, "service_dir"=> $profile_dir));
                        }
                    }
                    catch(Exception $e){
